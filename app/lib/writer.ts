...
import { validateArticle, checkTitle, quoteSourceAllowed, type GeneratedArticle } from './validation';
...
async function stepSchrijf(topic: Topic, s: StandaardState): Promise<StandaardStepResult> {
  ...
  try {
    ...
    validateArticle(candidate, subjectName(topic, s), constraints, { sparse });
...
async function stepSchrijfRetry(topic: Topic, s: StandaardState): Promise<StandaardStepResult> {
  ...
  try {
    ...
    validateArticle(checked, subjectName(topic, s), constraints, { sparse });
  } catch (e: any) {
