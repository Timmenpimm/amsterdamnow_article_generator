...
import { askClaudeJsonWithImages } from '@/lib/claude';
import { listImageCandidates, unscoredImageCandidates, scoreImageCandidate } from '@/lib/db';
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!batch.length) {
      return NextResponse.json({ candidates: await listImageCandidates(article.id), remaining: 0 });
    }
